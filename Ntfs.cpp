...
int Ntfs::check(const char *fsPath) {
    ...
    if (!WIFEXITED(status)) {
        SLOGE("Filesystem (ntfs) check did not exit properly");
        errno = EIO;
        return -1;
    }

    if ((status == 0) || (status == 3840)) {
        SLOGI("Filesystem (ntfs) checked OK");
        return 0;
    } else {
        SLOGE("Check (ntfs) failed (unknown exit code %d)", status);
        errno = EIO;
        return -1;
    }
...
int Ntfs::doMount(const char *fsPath, const char *mountPoint,
 ...
 //  flags = MS_NODEV | MS_NOSUID | MS_DIRSYNC;
	...
	if( rc !=0 )
	{
		SLOGE("Ntfs::doMount error :", strerror(errno));
        errno = EIO;
        return -1;
	}

    if (!WIFEXITED(status)) {
        SLOGE("Filesystem (ntfs) format did not exit properly");
        errno = EIO;
        return -1;
    }

    if (status == 0) {
        SLOGI("Filesystem (ntfs) Mounted OK");
        return 0;
    } else {
        SLOGE("Mount (ntfs) failed (unknown exit code %d)", status);
        errno = EIO;
        return -1;
    }
